...
The next lowest note, A♭3, is about as rare and is found in 10 hymns, including <hymn>I Stand All Amazed (#193)</hymn>, <hymn>As Now We Take the Sacrament (#169)</hymn>, and <hymn>A Poor Wayfaring Man of Grief (#29)</hymn>. Since the sacrament hymns that fall into this category are common and most of the non-sacrament hymns are not very common, it's primarily during the sacramet hymns that you see these low A♭s. 
...
There are 64 hymns that have the alto line go down to an A3. Some of these include sacrament hymns like <hymn>How Great the Wisdom and the Love (#195)</hymn> and <hymn>O God, the Eternal Father (#175)</hymn> and other very common hymns like <hymn>I Need Thee Every Hour (#98)</hymn>, <hymn>Hark! the Herald Angels Sing (#209)</hymn>, and <hymn>Count Your Blessings (#241)</hymn>.


```{r}
alto %>%
    filter(note_label == "Bb3") |> 
    pull(hymn_num) |> 
    single_hymn_lookup()
highest_lowest_note_stats("alto", "A3", "low")
```
...
freqs %>%
    ...
    cleveland(title = "Frequency of hymns with soprano lines that have Eb3 as the lowest note",
                   subtitle = NULL,
                   breaks = 0.5,
                   x_buffer = 1.2)
```
...
freqs %>%
    ...
    cleveland(title = "Frequency of hymns with soprano lines that have E4 as the lowest note",
                   subtitle = NULL,
                   breaks = 0.2,
                   x_buffer = 0.3)
```
...
freqs %>%
    ...
    cleveland(title = "Frequency of hymns with soprano lines that have F4 as the lowest note",
                   subtitle = NULL,
                   x_buffer = 1)
```
