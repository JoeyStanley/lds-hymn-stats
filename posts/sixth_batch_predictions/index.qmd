...
weekly_results <- name_num_lookup |> 
    ...
    mutate(times_sung = map_int(hymn_num, get_n_times_sung),
           sundays_since_release = floor(as.numeric((batch6_release_date - release_date)/7)),
...
```{r}
batch6_actual <- name_num_lookup |> 
    filter(hymn_num %in% batch6_hymn_nums) |> 
    mutate(hymn_num = as.integer(hymn_num)) |> 
    
    # Basic info
    mutate(batch = "batch6",
           release_date = batch6_release_date) |> 
    
    # Get the basic info
    mutate(times_sung = map_int(hymn_num, get_n_times_sung),
           sundays_since_release = floor(as.numeric((today() - release_date)/7)),
           meetings_since_release = map_int(release_date, get_n_meetings_since_release)) |> 
...
```{r, include=TRUE}
predicted_df <- batch6_preds |> 
    arrange(-prop_wards) |> 
    select(hymn_num, `predicted` = prop_wards)

actual_df <- batch6_actual |> 
    select(hymn_num, `actual` = prop_wards)

pred_vs_actual <- actual_df |> 
    left_join(predicted_df, by = "hymn_num") |> 
...
pred_vs_actual |> 
    mutate(label = paste0(hymn_name, " (", hymn_num, ")"),
           across(c(actual, predicted, difference), ~paste0(str_pad(round(. * 100, 2), 
                                                    width = 4, side = "right", pad = "0"), "%"))) |> 
    select(Hymn = label, predicted, actual, difference) |> 
    gt() |> 
    tab_header(title = "Predicted percent of sacrament meetings and actual percent of sacrament meeting each hymn from the 6th batch of hymns",
               subtitle = "Predictions are made February 27th based on frequency information for the first five batches of hymns. Actual results based on data collected through July 23rd.") |> 
    opt_align_table_header(align = "left") |> 
    tab_footnote(footnote = "For context, the most popular new hymns range between 1% and 3% of sacrament meetings.")
...
In June, I predicted that <hymn>`r hymn(1052)`</hymn> would be the most common, in large part because it's tagged as a sacrament hymn. Next was <hymn>`r hymn(1060)`</hymn>, partly because it had already been released in a church publication before. As it turns out though, the most commonly sung hymn from the sixth batch was <hymn>`r hymn(1061)`</hymn>. I suspect it was because it was somewhat popular around Mother's Day and Father's Day. But, once we get data from a full year, the influence of those two Sundays will level out, and it might be overtaken by another hymn. The next most common hymn was <hymn>`r hymn(1062)`</hymn>, which saw a small spike on each Fast Sunday. I didn't anticipate either of those when I made my predictions and I'll have to incorporate them into future predictions.


```{r, include = FALSE}
# Not a strong correlation I don't think.
pred_vs_actual |> 
    ggplot(aes(predicted, actual)) + 
    geom_point() + 
    geom_abline() + 
    geom_text(aes(label = hymn_name), nudge_x = 0.0001, hjust = 0) + 
    scale_x_continuous(labels = label_percent()) + 
    scale_y_continuous(labels = label_percent())

# This makes sense I think because of just a floor effect.
# I don't think this really says anything.
pred_vs_actual |> 
    ggplot(aes(predicted, difference)) + 
    geom_point() + 
    geom_text(aes(label = hymn_name), nudge_x = 0.0001, hjust = 0) + 
    scale_x_continuous(labels = label_percent()) + 
    scale_y_continuous(labels = label_percent())
...
While I didn't get the top two right, I did accurately predict the five or so least commonly sung hymns. <hymn>`r hymn(1053)`</hymn>, <hymn>`r hymn(1054)`</hymn>, <hymn>`r hymn(1056)`</hymn>, <hymn>`r hymn(1057)`</hymn>, (unsurprisingly since it's an Easter hymn) <hymn>`r hymn(1210)`</hymn>, and to a lesser extent <hymn>`r hymn(1058)`</hymn> were predicted to be unpopular, and that turned out to be true. These were predicted to be less popular because they were either hymns borrowed from other denominations (which tend to be less popular than original Latter-day Saint compositions), because they're arranged in unison (and are sometimes seen by some people as being "for kids" and therefore not appropriate for sacrament meeting), have a nontraditional flow, and/or were not previously published. 

Something that is definitely noteworthy is that, with the exception of <hymn>`r hymn(1061)`</hymn>, all hymns were sung less often than I expected. In my predictions, I did say outright that all the numbers are likely inflated because each batch seems to be less common than the one previous. So, I did get that part of my prediction right. In retrospect, I should have accounted for this difference by subtracting each prediction by some amount, but I didn't really know how to calcuate what what would be. So, I'll have to take that into account for the next batch.
...
```{r}
make_new_hymns_plot <- function(.df, .title) {
    .df |> 
    arrange(prop_wards) |> 
    mutate(label = paste0(hymn_name, " (", hymn_num, ")"),
           label = fct_inorder(label)) |> 
...
weekly_results |> 
    ...
    make_new_hymns_plot(.title = "Predicted popularity of the sixth batch of hymns")
```
...
```{r, include = TRUE, fig.height = 9, fig.width = 8}
weekly_results |> 
    bind_rows(batch6_actual) |>
    make_new_hymns_plot(.title = "Actual popularity of the sixth batch of hymns")
```
...
In this post, I took frequency data from the first five batches to make predictions about the popularity of the sixth batch of hymns. The statistics suggest that <hymn>Joyfully Bound (#1052)</hymn> and <hymn>Build an Ark (#1060)</hymn> would be the most popular from this batch. In reality, <hymn>`r hymn(1061)`</hymn> and <hymn>`r hymn(1062)`</hymn> were the most common. However, most of my guesses were fairly close, although they were almost all inflated by a little bit. I look forward to learning from this and making more accurate predictions for the seventh batch!