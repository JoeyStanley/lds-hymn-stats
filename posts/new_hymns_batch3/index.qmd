...
prop_new_batch3_week3 <- data_to_plot |> 
    ...
    pull(prop_sung) |> 
    as_percent_joey()
prop_new_batch3_week4 <- data_to_plot |> 
    filter(date == "2025-03-09",
           batch == "batch3") |>
    pull(prop_sung) |> 
    as_percent_joey()
```
...
Even though it has been just a few weeks, I've seen almost all the new hymns sung at least once in my dataset. It looks like <hymn>Softly and Tenderly Jesus Is Calling (#1020)</hymn> is currently the most popular of the new hymns, but the order will likely change over the next few weeks and months. 
