...
Another way of looking at the data is to consider how many new hymns a ward typically sang in the 15 weeks between the first and second batches. Did most wards sing one a week? Just one? What is the typical number of new hymns a ward has sung during this period?

It's hard to say for sure with the data I have. A lot of my data comes from single-week contributions from various wards. Many of the other wards in this sample contributed multiple weeks, but might have missed one or two because of stake conference or whatever other reason. So, if I've only got ten weeks of data from a particular ward during this 15 week period, and in none of those 10 did they sing new hymns, it may be the case that I just happened to not capture them when they did. 
...
So, I decided to look at all the data I have and organize it by how many weeks' worth of data I have from each ward. For eac
...
news_per_ward |> 
    ...
    filter(n_new < 19) |> 
    ggplot(aes(n_weeks, n_new, color = prop_per_n)) + 
    geom_point(shape = 15, size = 11, position = position_nudge(x = -0.1)) + 
    geom_point(shape = 15, size = 11, position = position_nudge(x =  0.1)) +
    geom_text(aes(label = prop_label), color = "white") + 
    stat_smooth(method = "lm", color = "#3f007d") +
