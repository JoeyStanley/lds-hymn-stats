...
This post is based primarily on `r comma(length(unique(freqs_easters$meeting_id)))` sacrament meetings from `r length(unique(freqs_easters$ward))` wards since 2001. You can read more about the full dataset [here](/posts/frequency-data).
...
#| label: fig-cleveland
around_easter_data <- freqs_easters |>
    filter(easter %in% c("easter", "week before", "week after", "two weeks before"),
...
around_easter_data |> 
    ...
    labs(title = "Percentage of wards that sing each Easter hymn per year",
                   subtitle = "From two weeks before to a week after Easter") + 
...
palm_sunday |>
    cleveland_plot(filter_n = 15, 
                   ...
                   # n_weeks = n_dates/7,
                   # unique_meetings = nrow(data_to_plot),
