...
This post is based primarily on 1,045 sacrament meetings from 288 wards spanning 21 years. You can read more about the full dataset [here](/posts/frequency-data).
...
freqs_easters <- freqs |>
    mutate(date_date = ymd(date),
           easter = case_when(date_date %in%   easters$date ~ "easter", 
                              ...
                              date_date %in% c(easters$date - 28) ~ "four weeks before",
                              date_date %in% c(easters$date - 35) ~ "five weeks before",
                              date_date %in% c(easters$date - 42) ~ "six weeks before",
                              date_date %in% c(easters$date +  7) ~ "week after",
                              date_date %in% c(easters$date + 14) ~ "two weeks after",
                              TRUE ~ "not Easter"),
           easter = factor(easter, levels = c("six weeks before", "five weeks before", "four weeks before", "three weeks before", "two weeks before", "week before", "easter", "week after", "two weeks after", "not Easter")),
...
#| fig-cap: Proportion of sacrament meetings with Easter hymns around Easter
...
```{r, include = TRUE, fig.height = 4, warning=FALSE}
...
# First draft, where I looked at raw number of hymns.
# First, here is a plot showing the number of "Easter" hymns across the whole year. 
freqs_easters |> 
    filter(hymn_num %in% all_easter_hymns) |> 
    count(week) |> 
    ggplot(aes(week, n)) + 
    geom_col() + 
    scale_x_continuous(breaks = seq(0, 100, 10)) + 
    labs(title = "Easter Hymns by Week Number",
         x = "Week Number",
         y = "Number of hymns sung (in the current databse)") + 
    theme_minimal()
```
...
* The 32nd through 38th weeks of the year, which corresponds to August 5 through September 23. There are no major holidays during that period. It's after Pioneer Day (in Utah) and before when [General Conference](/posts/general_conference.qmd) hymns pick up. 
...
```{r, include = FALSE, eval = FALSE, fig.height = 4}
# First draft
freqs_easters |> 
    filter(hymn_num %in% all_easter_hymns) |> 
    count(month, week) |> 
    mutate(in_range = case_when(month %in% c(32:38) ~ TRUE,
                                week %in% c(2:5, 22:23, 32:38) ~ TRUE,
                                TRUE ~ FALSE)) |> 
    ggplot(aes(week, n, fill = in_range)) + 
    geom_col() + 
    scale_x_continuous(breaks = seq(0, 100, 10)) + 
    scale_fill_manual(breaks = c("TRUE", "FALSE"),
                     values = c(ptol_blue, "gray75")) +
...
freqs_easters |> 
    ...
    mutate(prop_easter = `TRUE`/sum(c(`TRUE`, `FALSE`)), .by = easter) |>
    mutate(rel_week = as.numeric(easter) - 7) |>
    ggplot(aes(rel_week, prop_easter, fill = prop_easter > 0.27)) + 
    geom_col() + 
    geom_hline(yintercept = 0.27, color = "gray50", linetype = "dashed") + 
    scale_x_continuous(breaks = -10:10) + 
    scale_y_continuous(expand = expansion(0, c(0, 0.02)),
                       minor_breaks = seq(0, 100, 0.05),
                       labels = scales::percent) + 
...
```{r, include = TRUE, fig.height = 5, fig.width = 6.5, message=FALSE}
#| fig-cap: Percentage of wards that sing each Easter hymn per year
#| label: fig-cleveland
freqs_easters |>
    filter(easter %in% c("easter", "week before", "week after", "two weeks before"),
           hymn_num %in% all_easter_hymns) |> 
    cleveland_plot(title = "Percentage of wards that sing each Easter hymn per year",
                   subtitle = "This only includes from two weeks before to a week after Easter") + 
    scale_x_continuous(name = "percent", 
                       labels = scales::percent, 
                       expand = expansion(0, c(0, 0.25)),
                       breaks = seq(0, 1, 0.1))
```

We can immediately see that <hymn>He Is Risen! (#199)</hymn> and <hymn>Christ the Lord is Risen Today (#200)</hymn> are far and away the most common hymns during this time, with the former just slightly ahead but the difference is so small we could probably consider them being in joint first place. What is the most surprising to me is how low those percentages are: each of those hymns is sung by the congregation in only about 58% of wards each year!But, like what we saw with the [Christmas](/posts/christmas) hymns, it's very possible that many more wards *hear* these hymns each year just as musical numbers of some sort, rather than as a congregational hymn. 
...
There are a few interesting patterns here. For one, the very infrequent hymns, <hymn>Lo, the Mighty God Appearing (#55)</hymn> and <hymn>The Glorious Gospel Light Has Shone (#283)</hymn>, appear to peak after or before Easter, but it's important to keep in mind that each of those was only sung once in the Easter season in my database, so we shouldn't draw too many strong conclusions about those.
...
We can look at the same data a slightly different way and focus on one week at a time. We'll start with Pqlm Sunday. We just mentioned above that <hymn>All Glory, Laud, and Honor (#69)</hymn> shows a very clear peak the week before Easter. Is it the most common hymn on Palm Sunday? @fig-cleveland_palm shows that it is actually not. Five other hymns are simply more common overall, so even though a hymn like <hymn>He is Risen! (#199)</hymn> is much more common on Easter than on Palm Sunday, the sheer fact that it's sung so much more in general means that even when it's not at its peak, it's still more common than <hymn>All Glory, Laud, and Honor (#69)</hymn> on its peak. Palm Sunday also has competition with [General Conference](/posts/general_conference.qmd), so <hymn>We Thank Thee, O God, for a Prophet (#19), even wins out on Palm Sunday. Overall, only 5.5% of wards sung <hymn>All Glory, Laud, and Honor (#69)</hymn> on Palm Sunday.
...
freqs_easters |>
    ...
    mutate(is_easter = hymn_num %in% all_easter_hymns) |> 
    cleveland_plot(title = "Frequency of hymns on Palm Sunday",
...
freqs_easters |>
    ...
    mutate(is_easter = hymn_num %in% all_easter_hymns) |> 
    cleveland_plot(title = "Frequency of hymns two weeks before Easter",
                   subtitle = "Easter hymns are highlighted in <span style = 'color: #4477AA;'>blue</span>",
                   filter_n = 8,
                   extra_cols = is_easter,
                   color = is_easter) + 
...
We now move on to Easter Sunday itself. @fig-cleveland_easter show the most commonly sung hymns on Easter. This looks similar to @fig-cleveland, but here we're just looking at Easter Sunday but we're expanding it to include all hymns, regardless of whether they're "technically" Easter hymns. Here we see that all the hymns on this list are Easter hymns as discussed above or are Sacrament hymns. It's been hinted at already, but it's clear that the bulk of Easter singing happens on Easter. As is evident in this plot, Easter hymns dominate on Easter Sunday, and as we saw already in @fig-weeks_around_easter, 98.4% of wards sing at least one Easter hymn that week.
...
freqs_easters |>
    ...
    mutate(is_easter = hymn_num %in% all_easter_hymns) |> 
    cleveland_plot(title = "Frequency of hymns on Easter Sunday",
                   subtitle = "Easter hymns are highlighted in <span style = 'color: #4477AA;'>blue</span>",
                   filter_n = 7,
                   extra_cols = is_easter,
                   color = is_easter) + 
...
The two other most canonical Easter hymns, <hymn>That Easter Morn (#198)</hymn> and <hymn>O Savior, Thou Who Wearest a Crown (#197)</hymn>, are less common, being sung by 13.8% and 5.1% of wards each year on Easter, respectively. Most of the rest of the hymns on this list are sacrament hymns. The four sacrament hymns listed in the Easter entry of the Index are the most common, with the exception of <hymn>There is a Green Hill Far Away (#194)</hymn>, which has clear reference to the crucifixion. As for the other hymns, <hymn>I Know that My Redeemer Lives (#136)</hymn>, <hymn>All Creatures of Our God and King (#62)</hymn>, and <hymn>I Believe in Christ (#134)</hymn> are the only others on this list. 
...
freqs_easters |>
    ...
    mutate(is_easter = hymn_num %in% all_easter_hymns) |> 
    cleveland_plot(title = "Frequency of hymns on the Sunday after Easter",
                   subtitle = "Easter hymns are highlighted in <span style = 'color: #4477AA;'>blue</span>",
                   filter_n = 11,
                   extra_cols = is_easter,
                   color = is_easter) + 
...
freqs_easters |>
    ...
    labs(title = "When are the two most popular Easter hymns sung in the meeting?",
         subtitle = "They're virtually identical!",
         y = "count") + 
    theme_bw()
```
