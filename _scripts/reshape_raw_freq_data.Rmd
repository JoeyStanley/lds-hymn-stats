...
Yvonne Dixon sent this (Oct 9, 2023) based on a facebook comment. She said they only listened to the sacrament hymns and did not sing, so in most of the 2020 data, there is just a sacrament hymn. Then there were a few months in 2021 without sacrament hymns for some reason.

```{r}
willis_point_2020 <- read_excel("../data/wards/Wills Point Ward.xlsx", sheet = "2020", 
                         col_names = c("date", "opening", "opening_name", "sacrament", "sacrament_name", "closing", "closing_name")) %>%
    clean_names() %>%
    select(date, opening, sacrament, closing) %>%
    print()
willis_point_2021 <- read_excel("../data/wards/Wills Point Ward.xlsx", sheet = "2021") %>%
    clean_names() %>%
    select(date = x2, opening, sacrament, `intermediate` = rest_hymn, closing) %>%
    mutate(opening = as.numeric(opening)) %>%
    print()
willis_point_2022 <- read_excel("../data/wards/Wills Point Ward.xlsx", sheet = "2022") %>%
    clean_names() %>%
    select(date = x2, opening, sacrament, `intermediate` = special_number, closing) %>%
    filter(!is.na(date)) %>%
    mutate(across(c(opening, intermediate), as.numeric)) %>%
    print()
willis_point_2023 <- read_excel("../data/wards/Wills Point Ward.xlsx", sheet = "2023") %>%
    clean_names() %>%
    select(date = x2, opening, sacrament, `intermediate` = special_number, closing) %>%
    mutate(across(c(opening, intermediate), as.numeric)) %>%
    print()

willis_point <- bind_rows(willis_point_2020, willis_point_2021, willis_point_2022, willis_point_2023) %>%
    pivot_longer(cols = -date, names_to = "type", values_to = "hymn_num") %>%
    filter(!is.na(hymn_num)) %>%
    mutate(date = as.Date(date)) %>%
    left_join(name_num_lookup, by = "hymn_num") %>%
    
     # Add the extra stuff
    mutate(ward = "Wills Point",
           city = "Wills Point",
           state = "Texas",
           country = "United States") %>%
    print()
...
Wendy Hickman sent this to me October 9, 2023. Probably the trickiest format I've seen, but it has data from 2017 through 2023!

```{r}
ssh <- read_csv("../data/wards/ssh3.csv", show_col_types = FALSE) %>%
    clean_names() %>%
    select(date = x1, hymns) %>%
    ...
    # Now the hymns
    filter(!is.na(hymns)) %>%
    mutate(hymns = str_remove_all(hymns, "[^0-9\\s]"), # strip everything but numbers and spaces
           hymns = str_remove(hymns, "\\A\\s+"), # strip leading spaces
           hymns = str_remove(hymns, "\\s+\\Z"), # strip trailing spaces
           hymns = str_replace_all(hymns, "\\s+", " ")) %>%  # consolidate spaces
    separate(hymns, into = c("opening", "sacrament", "other"), extra = "merge") %>%
    separate(other, into = c("intermediate", "closing"), fill = "left") %>%
    ...
    mutate(ward = "SSH",
           ...
           state = "Unknown",
           country = "United States") %>%
    mutate(hymn_num = as.double(hymn_num)) %>%
    print()
```

Note that there a few warnings introduced in the separate function from other to intermediate and closing. I checked and they're all legit. 
...
rm(marion, 
   ...
   spanish_trails,
   fort_meade,
   cedarcitySA, cedarcitySA_2019, cedarcitySA_2020, cedarcitySA_2021,
   brenner_pass,
   mm2_2023, mm2_2022, mm2,
   willis_point_2020, willis_point_2021, willis_point_2022, willis_point_2023, willis_point,
   ssh,
   misc)
```
