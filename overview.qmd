...
I currently have data from `r scales::comma(get_n_distinct_meetings())` sacrament meetings. If you were to attend that many, it would take about `r scales::comma(round(get_n_distinct_meetings()/48, 0))` years---more than a single person can experience in a lifetime. This data comes from `r scales::comma(length(unique(freqs$ward)))` wards, mostly in the United States. Across all these meetings, there are `r scales::comma(nrow(freqs))` hymns. See [here](/posts/frequency-data) for a detailed breakdown of how this data is distributed across time and space, as well as an explanation of where it call came from. 
...
# This version of the table is no longer used because it has been more than a year. Removed as of January 5, 2026. The coloring was cool but I don't want to bother redoing it. 
...
post_june_data <- freqs %>%
    ...
    # don't show children's songs or non-English songs in this section
...
get_rows_of_new_hymns <- function(.hymn_nums) {
    post_june_data |> 
        rowid_to_column("real_row") |> 
        filter(as.numeric(str_remove(hymn_num, "#")) %in% .hymn_nums) |> 
        pull(real_row)
}
batch1_rows <- get_rows_of_new_hymns(batch1_hymn_nums)
batch2_rows <- get_rows_of_new_hymns(batch2_hymn_nums)
batch3_rows <- get_rows_of_new_hymns(batch3_hymn_nums)
batch4_rows <- get_rows_of_new_hymns(batch4_hymn_nums)
batch5_rows <- get_rows_of_new_hymns(batch5_hymn_nums)
batch5_rows <- get_rows_of_new_hymns(batch6_hymn_nums)
...
post_june_data |> 
    ...
    row_spec(row = batch1_rows, background = "#b3cde3") |> # colorbrewer Pastel1
    ...
    row_spec(row = batch5_rows, background = "#fed9a6") |> 
    row_spec(row = batch5_rows, background = "#ffffcc")
```
